/**
 * Additional tests for TicketForm covering the assignee input handler.
 *
 * カバレッジ対象:
 *   - TicketForm.tsx assignee input onChange (setAssignee)
 *   - 送信データ中の assignee 値 (trim / 空文字時に undefined)
 */
...
const validDescArb = fc
  ...
  .filter((s) => s.trim().length > 0);
const validAssigneeArb = fc
  .string({ minLength: 1, maxLength: 50 })
  .filter((s) => s.trim().length > 0);

describe('TicketForm - assignee input', () => {
  it('assignee input に入力した値が state に反映される', () => {
    fc.assert(
  ...
  it('assignee を入力してsubmitすると trim された値が渡される', () => {
    fc.assert(
  ...
  it('assignee が空文字/空白のみのときは undefined として送信される', () => {
    fc.assert(
      fc.property(
        validTitleArb,
        validDescArb,
